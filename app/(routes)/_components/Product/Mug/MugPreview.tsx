...
const MugPreview = ({ configuration }: MugPreviewProps) => {

    const { id } = configuration
    const { mugColor, basePrice, totalPrice } = configuration
    const color = MCOLORS.find((supportedcolor) => supportedcolor.value === mugColor)?.timage
    ...
    return (
     ...
     md:grid-cols-12 gap-8'>
                <div className='md:col-span-4'>
                    <MugDesign src={color} imgSrc={configuration.croppedImageUrl} />
                </div>
                <div className='md:col-span-8'>
                    ...
                    <div className='mt-8 flex justify-end space-x-4'>
                        {!isSignedIn && !user ? (
                            <ModalLogin redirectUrl={`/catalog/${id}/mug/preview`} />
                        ) :
                            <Button onClick={() => router.push(`/catalog/${id}/mug/checkout`)} disabled={!isSignedIn} className=''>
